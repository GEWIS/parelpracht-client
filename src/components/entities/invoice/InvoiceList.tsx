  ...
  if (invoices.length === 0) {
    return (
      <>
        ...
        <h4>{t('entities.product.noInvoice')}</h4>
      </>
    );
  }
      ...
      <Table compact>
        <Table.Header>
          <Table.Row>
            ...
            <Table.HeaderCell>{t('entities.generalProps.amount')}</Table.HeaderCell>
            <Table.HeaderCell>{t('entities.generalProps.status')}</Table.HeaderCell>
            <Table.HeaderCell>{t('entities.invoice.props.financialYear')}</Table.HeaderCell>
...
export default withTranslation()(withRouter(connect(mapStateToProps, mapDispatchToProps)(InvoiceList)));
