import React from 'react';
import { Loader, Table } from 'semantic-ui-react';
import {
  Client, PaginationParams, Product, ProductInstance,
} from '../../clients/server.generated';
import TablePagination from '../TablePagination';
import ContractCompactRow from '../contract/ContractCompactRow';
import InvoiceCompactRow from '../invoice/InvoiceCompactRow';

interface Props {
  product: Product;
  header: string;
  attribute: 'invoice' | 'contract';
}

interface State {
  productInstances: ProductInstance[];
  countTotal: number;
  loading: boolean;

  skip: number;
  take: number;
}

class ContractCompactTable extends React.Component<Props, State> {
  constructor(props: Props) {
    super(props);
    this.state = {
      productInstances: [],
      countTotal: 0,
      skip: 0,
      take: 20,
      loading: false,
    };
  }

  async componentDidMount() {
    await this.getProductAttributes();
  }

  async getProductAttributes() {
    const { skip, take } = this.state;
    const { product, attribute } = this.props;
    this.setState({ loading: true });
    const client = new Client();

    let productInstances;
    if (attribute === 'contract') {
      productInstances = await client.getProductContracts(product.id, {
        skip,
        take,
      } as PaginationParams);
    } else if (attribute === 'invoice') {
      productInstances = await client.getProductInvoices(product.id, {
        skip,
        take,
      } as PaginationParams);
    }
    if (productInstances === undefined) throw new TypeError('productInstances undefined');

    this.setState({
      productInstances: productInstances.list,
      countTotal: productInstances.count,
      loading: false,
    });
    this.render();
  }

  nextPage = async () => {
    const { skip, take, countTotal } = this.state;
    if (skip + take <= countTotal) {
      this.setState({ skip: skip + take });
      await this.getProductAttributes();
    }
  };

  prevPage = async () => {
    const { skip, take } = this.state;
    if (skip - take >= 0) {
      this.setState({ skip: skip - take });
      await this.getProductAttributes();
    }
  };

  setTake = async (take: number) => {
    await this.setState({
      take,
      skip: 0,
    });
    await this.getProductAttributes();
  };

  render() {
    const {
      productInstances, countTotal, skip, take, loading,
    } = this.state;
    const { header, attribute } = this.props;

    let body;
    if (attribute === 'contract') {
      body = (
        <Table.Body>
          {productInstances.map((p) => <ContractCompactRow contract={p.contract} />)}
        </Table.Body>
      );
    } else if (attribute === 'invoice') {
      console.log(productInstances);
      const temp = productInstances.filter((p) => p.invoiceId !== null);
      console.log(temp);
      body = (
        <Table.Body>
          {temp.map((p) => <InvoiceCompactRow invoice={p.invoice} />)}
        </Table.Body>
      );
    }

    return (
      <>
        <h3>{header}</h3>
        <Table>
          <Loader active={loading} />
          {body}
        </Table>
        <TablePagination
          countTotal={countTotal}
          countFetched={productInstances.length}
          skip={skip}
          take={take}
          nextPage={this.nextPage}
          prevPage={this.prevPage}
          setTake={this.setTake}
        />
      </>
    );
  }
}

export default ContractCompactTable;
