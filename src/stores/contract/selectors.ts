...
export function sortColumn(state: RootState): string {
  ...
  switch (column) {
    ...
    case 'title':
      return i18n.t('entities.contract.props.title').toLowerCase();
    case 'company':
      return i18n.t('entity.company').toLowerCase();
    case 'contact':
      return i18n.t('entity.contact').toLowerCase();
    case 'updatedAt':
...
export function getContractTitle(state: RootState, id: number): string {
  return getSummary<ContractSummary>(state, SummaryCollections.Contracts, id)?.title ?? '...';
}

export function getContractStatus(state: RootState, id: number): ContractStatus {
  return getSummary<ContractSummary>(state, SummaryCollections.Contracts, id)?.status ?? ('...' as ContractStatus);
}

export function getContractValue(state: RootState, id: number): number {
  return getSummary<ContractSummary>(state, SummaryCollections.Contracts, id)?.value ?? undefined;
}
