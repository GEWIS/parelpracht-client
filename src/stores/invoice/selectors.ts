...
export function sortColumn(state: RootState): string {
  ...
  switch (column) {
    ...
    case 'title':
      return i18n.t('entities.invoice.props.title').toLowerCase();
    case 'company':
      return i18n.t('entity.company').toLowerCase();
    case 'startDate':
      return i18n.t('entities.invoice.props.financialYear').toLowerCase();
    case 'updatedAt':
      return i18n.t('entities.generalProps.lastUpdate').toLowerCase();
    default:
...
export function getInvoiceTitle(state: RootState, id: number): string {
  return getSummary<InvoiceSummary>(state, SummaryCollections.Invoices, id)?.title ?? '...';
}

export function getInvoiceStatus(state: RootState, id: number): InvoiceStatus {
  return getSummary<InvoiceSummary>(state, SummaryCollections.Invoices, id)?.status ?? ('...' as InvoiceStatus);
}

export function getInvoiceValue(state: RootState, id: number): number {
  return getSummary<InvoiceSummary>(state, SummaryCollections.Invoices, id)?.value ?? undefined;
}
