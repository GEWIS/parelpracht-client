...
export function sortColumn(state: RootState): string {
  ...
  switch (column) {
    ...
    case 'name': return i18n.t('entities.company.props.name').toLowerCase();
    case 'status': return i18n.t('entities.generalProps.status').toLowerCase();
    case 'updatedAt': return i18n.t('entities.generalProps.updatedAt').toLowerCase();
...
export function sortColumnMegaTable(state: RootState): string {
  ...
  switch (column) {
    case 'id': return i18n.t('entities.generalProps.ID');
    case 'companyName': return i18n.t('entity.company').toLowerCase();
    default: return i18n.t('entities.generalProps.unknown').toLowerCase();
  }
}
