...
export function sortColumn(state: RootState): string {
  ...
  switch (column) {
    ...
    case 'nameDutch':
      return i18n.t('entities.product.props.nameNl').toLowerCase();
    case 'nameEnglish':
      return i18n.t('entities.product.props.nameEn').toLowerCase();
    case 'targetPrice':
      return i18n.t('entities.product.props.price').toLowerCase();
    case 'status':
      return i18n.t('entities.generalProps.status').toLowerCase();
    case 'category':
      return i18n.t('entity.category').toLowerCase();
    default:
...
export function getProductName(state: RootState, id: number): string {
  ...
  if (currentLanguage === 'nl-NL') {
    return getSummary<ProductSummary>(state, SummaryCollections.Products, id)?.nameDutch ?? '...';
  }
  return getSummary<ProductSummary>(state, SummaryCollections.Products, id)?.nameEnglish ?? '...';
}
